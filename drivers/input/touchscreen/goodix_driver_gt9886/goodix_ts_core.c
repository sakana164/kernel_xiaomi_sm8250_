	...
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry_safe (ext_module, next,
			...
			if (ext_module->funcs && ext_module->funcs->exit)
				ext_module->funcs->exit(
					goodix_modules.core_data, ext_module);
		}
	}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&goodix_modules.mutex);
	if (!list_empty(&goodix_modules.head)) {
		list_for_each_entry_safe (module, next, &goodix_modules.head,
					  list) {
...
static struct attribute *sysfs_attrs[] = {
	&dev_attr_extmod_info.attr,    &dev_attr_driver_info.attr,
	&dev_attr_chip_info.attr,      &dev_attr_reset.attr,
	&dev_attr_send_cfg.attr,       &dev_attr_read_cfg.attr,
	&dev_attr_irq_info.attr,       &dev_attr_reg_rw.attr,
	&dev_attr_tp_test.attr,	       &dev_attr_tp_rawdata.attr,
	&dev_attr_tp_get_testcfg.attr, NULL,
};
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (ts_bdata->vddio_name) {
		core_data->vddio =
			devm_regulator_get(dev, ts_bdata->vddio_name);
...
void goodix_sync_setup(struct goodix_ts_core *core_data)
{
	...
	sync_try_time = 1;
	while (sync_try_time--) {
		ts_dev->hw_ops->write(core_data->ts_dev, 0x4160,
				      sync_enable_data, 5);
		msleep(150);
		ts_dev->hw_ops->write(core_data->ts_dev, 0x4160,
				      sync_base_update, 5);
					   ...
					   size_t count, loff_t *pos)
{
	...
	if (*pos != 0 || !goodix_core_data)
		return 0;
	r = goodix_core_data->ts_dev->hw_ops->read_version(
		goodix_core_data->ts_dev, &fw_ver);
	if (r)
		return 0;
	...
	cnt = snprintf(k_buf, 100,
		       ...
		       fw_ver.vid[0], fw_ver.vid[1], fw_ver.vid[2],
...
static DEVICE_ATTR(fod_status, (S_IRUGO | S_IRGRP), goodix_fod_status_show,
		   NULL);

static DEVICE_ATTR(fod_test, (S_IRUGO | S_IWUSR | S_IWGRP), NULL,
		   goodix_fod_test_store);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	core_data->power_supply_notifier.notifier_call =
		goodix_power_supply_event;
	...
	core_data->tp_data_dump_proc = proc_create("tp_data_dump", 0444, NULL,
						   &goodix_rawdata_info_ops);
	core_data->tp_fw_version_proc = proc_create(
		"tp_fw_version", 0444, NULL, &goodix_fw_version_info_ops);
...
out:
	if (r) {
		...
		if (ts_device->board_data.irq_gpio) {
			gpio_direction_output(ts_device->board_data.irq_gpio,
					      0);
		}
		goodix_ts_power_off(core_data);
		...
		if (core_data->power_supply_notifier.notifier_call)
			power_supply_unreg_notifier(
				&core_data->power_supply_notifier);
