...
static void cvm_oct_rx_refill_worker(struct work_struct *work)
{
	...
	if (!atomic_read(&cvm_oct_poll_queue_stopping))
		queue_delayed_work(system_power_efficient_wq,&cvm_oct_rx_refill_work, HZ);
}
...
static void cvm_oct_periodic_worker(struct work_struct *work)
{
	...
	if (!atomic_read(&cvm_oct_poll_queue_stopping))
		queue_delayed_work(system_power_efficient_wq,&priv->port_periodic_work, HZ);
}
		...
		for (port_index = 0,
			...
			if (!dev->netdev_ops) {
				...
				fau -=
				    ...
				    sizeof(u32);
				queue_delayed_work(system_power_efficient_wq,&priv->port_periodic_work,
						      HZ);
			}
		}
	}
